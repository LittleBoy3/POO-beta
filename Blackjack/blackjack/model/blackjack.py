...
class Blackjack:
    ...
    def revelar_carta_oculta_casa(self):
        self.cupier.mano.revelar_carta_oculta()
    ...
    def casa_pedir_carta(self):
        if self.casa_puede_pedir():
            carta = self.baraja.repartir_carta()
            self.cupier.recibir_carta(carta)
            print(f"La casa pidió una carta: {carta}")
    ...
    def jugador_gano(self) -> bool:
        ...
        valor_mano_casa = self.cupier.mano.calcular_valor()
        try:
            valor_mano_jugador = int(valor_mano_jugador)
            valor_mano_casa = int(valor_mano_casa)
            return valor_mano_jugador == 21 or (valor_mano_jugador <= 21 and (valor_mano_casa > 21 or valor_mano_jugador > valor_mano_casa))
        except ValueError:
            return False
    ...
    def casa_gano(self) -> bool:
        ...
        try:
            valor_mano_jugador = int(valor_mano_jugador)
            valor_mano_casa = int(valor_mano_casa)
            return int(self.cupier.mano.es_blackjack()) or (valor_mano_casa > 21) or (valor_mano_jugador < valor_mano_casa <= 21)
        except ValueError:
            return False
